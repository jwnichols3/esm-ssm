...
#                   this program.
#                   
#           Usage:  fileage.monitor (use $SSM_ETC/fileage.dat*)
#                           (must run in $SSM_BIN)
#
# Revision History:
#
...
#
#  2005-06-08  wpd    2.60: Added code for check if a file exists.  If the 
#                           fileage configuration value = 0 then perform
#                           this check.
#
#####################################################################

$version             = "$0 version 2.60\n";
...
# ===================================================================
$DEBUG_EXTENSIVE_ON  = "$SSM_CONF" . "fileage.monitor.debugextensive";    # if this file is there then debug extensive is on
$DEBUG_ON            = "$SSM_CONF" . "fileage.monitor.debug";             # if this file is there then debug is on
...
$status_file = "$SSM_LOGS" . "fileage.monitor.status";
...
##
## Replacing the code following this with the function.
##
...
$running_status = chk_running("fileage.monitor");
...
foreach $config_file (@appl_files) {
   ...
   foreach $line (@fileageinfo) {
      ...
      if ( "$file" eq "\@" ) {
         ...
         $age1    = 0;
         print "Checking if the file $file exists\n";
         $any     = "N";
         chomp($DESC);
         if ( "$DESC" eq "" ) {
            $DESC = "$file";
      ...
      if ( $age == 0 ) {
         ...
         if (-e "$fname" ) {
            $errmsg_text = "The file $fname exists.";
            print "$errmsg_text\n\n";
            $running_err = "N";
            &chk_error;
         } else {
            $errmsg_text = "The file $fname is not found.  Check processing on server $HOSTNAME.";
            print "$errmsg_text\n\n";
            $running_err = "Y";
            &chk_error;
         }
      ...
      if ( "$any" eq "Y" || "$any" eq "A" ) {
         ...
         if ( "$found" ne "Y" ) {
            ...
            $errmsg_text = "No files in $DESC are less than $age minutes old on server $HOSTNAME.";
            $running_err = "N";
            print "$errmsg_text\n\n";
...
sub chk_file {
   ...
   if (($hour >= $start) && ($hour < $stop) && ($dw_found > 0)) {
      ...
      # if age threshold = 0 then check to see if a file exists
...
sub check_times {
   ...
   if ( $no_errors > $Error_Times && $Error_Times > 0 ) {
      ...
      $message = "Message from fileage.monitor monitoring via $config_file $disp_date $disp_time vposend_options: app=$appl sev=$severity message=$errmsg_text $cmd";
   ...
   if ("$running_err" eq "Y") {
      ...
      if (-e "$SSM_HOLD$file") {
         ...
         if ( $ageInMinutes > $ITO_AGE ) {
            ...
            $message = "Message from fileage.monitor monitoring via $config_file $disp_date $disp_time vposend_options: app=$appl sev=$severity message=$errmsg_text $action";
      ...
      if ( -e "$SSM_HOLD$file" ) {
         ...
         # print vposend ("Message from fileage.monitor monitoring $disp_date $disp_time __VPO__ app=$appl sev=normal message=$errmsg_text $action\n");
         # close (vposend);
      }
   }
