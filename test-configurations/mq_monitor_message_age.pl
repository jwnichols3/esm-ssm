...
## $Id: mq_monitor_message_age.pl,v 1.1 2005/08/30 17:18:39 nichj Exp $
...
foreach $todo ( @todo ) {
    ...
    if ( $count > 0 ) {
      ...
      if ( $alert_flg ) {
          ...
          log_message($todo{Who},$todo{Severity},$error_msg_txt,"",$todo{emailonly});
...
sub log_message {
    my($who,$priority,$message,$key,$emailaddress)=@_;
    my($string);
    ...
    if ( $emailonly ne "" ) {
    	$string.=" action=emailfyi,$emailaddress";
    }
...
## getting into.
##
## To support the option of just sending email messages to support teams the
##  emailonly option has been added (2005-05-12).  The usage:
##    emailonly=emailaddr@bgi.com;emailaddr2@bgi.com
##  Delimit email addresses with semicolons (;).
##
...
## SENSITIVE!
##
QMgr=QM.UK.P.01  Q=Q.TEST   Alert=5    Realert=60  Who=mqadmin Severity=minor  emailonly=testuser@barclaysglobal.com
##
EOF
...
sub show_version {
    ...
    print '$Id: mq_monitor_message_age.pl,v 1.1 2005/08/30 17:18:39 nichj Exp $'."\n";
